...
CREATE TABLE IF NOT EXISTS run(
    run_id SERIAL,
    auth.user_id UUID NOT NULL,--run_auth_user_id UUID NOT NULL REFERENCES auth.users(id)
    ...
    CONSTRAINT fk_run_auth_user_id  FOREIGN KEY (auth.user_id) REFERENCES auth.user_id
    CONSTRAINT fk_run_design_strategy_id    FOREIGN KEY (design_strategy_id)     REFERENCES design_strategy(design_strategy_id),
    CONSTRAINT uk_run_token UNIQUE (run_token)
);
