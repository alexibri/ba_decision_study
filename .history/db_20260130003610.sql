...
CREATE TABLE study_session(
    ...
    study_session_status VARCHAR(40) NOT NULL, --CHECK (study_session_status IN ('started','finished','aborted'))
...
CREATE TABLE screen(
    ...
    screen_button_choice VARCHAR(30) NOT NULL,
    screen_order_index INT NOT NULL,

    CONSTRAINT pk_screen_id PRIMARY KEY (screen_id),
    CONSTRAINT uk_screen_name_per_study UNIQUE (study_id, screen_name),
);
...
CREATE TABLE response_screen(
    ...
    CONSTRAINT fk_response_screen_id            FOREIGN KEY (screen_id)     REFERENCES screen(study_session_id) ON DELETE CASCADE,
);


--NEUESTES UPDATE: Response n:1 Screen. Damit hat offiziell 1 screen mehrere Responses