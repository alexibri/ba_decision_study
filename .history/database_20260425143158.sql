...
CREATE TABLE IF NOT EXISTS run (
    ...
    CONSTRAINT ck_run_status CHECK (
        LOWER(run_status) IN ('started', 'finished')
    )
);
...
CREATE TABLE IF NOT EXISTS response (
    ...
    CONSTRAINT ck_response_reaction_time_ms CHECK (
        response_reaction_time_ms >= 0
    )
);
...
CREATE UNIQUE INDEX uniq_run_user_finished ON run (user_id)
WHERE
    run_status = 'finished';

CREATE UNIQUE INDEX uniq_run_user_started ON run (user_id, design_strategy_id)
WHERE
    run_status = 'started';
...
CREATE POLICY "Users can read design strategies" ON design_strategy FOR
SELECT USING (true);

CREATE POLICY "Users can read screens" ON screen FOR
SELECT USING (true);
