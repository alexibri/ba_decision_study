...
export default function Start() {
    ...
    const startWithDesign = async ({currentDesignStrategy, groupName}) => {
        ...
        if (!runIdToUse) {
            const run = await createRun({ runStatus: "started", currentDesignStrategy});
            if (!run) return;
        ...
        <div className="start fade-in">
            <div className="s1screen">
                <div className="s1card">
                    <h1>Start</h1>
                    <div className="stack">
                        <button className="button" onClick={() => startWithDesign({currentDesignStrategydesignStrategy:1, groupName:"dark"})}>Dark Patterns</button>
