...
export default function S1Dark2() {
    ...
    const startTime = performance.now()
            ...
            const handleDark = async () => {
                ...
                const run = await createResponse({runID: localStorage.getItem("run_id"), screenID:2, selectedChoice:"Dark Pattern", rtms:performance.now() - startTime });
                if (!run) return;
            ...
            const handleNudge = async () => {
                const run = await createResponse({runID: localStorage.getItem("run_id"), screenID:2, selectedChoice:"Nudge", rtms:performance.now() - startTime });
                if (!run) return;
        ...
        <div className="s1card">
            ...
            <div className="s1scroll">
                ...
                <div className="s1button">
                    <button className="s1accept" onClick={() => navigate("/scenario/1/step/3")}>
                        Accept all
                    </button>
                </div>
            ...
            <div>
                <button className="s1decline" onClick={() => navigate("/scenario/1/step/3")}>
                    Decline all
                </button>
            </div>
