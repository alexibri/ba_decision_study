...
export default function S1Dark1() {
    ...
    const startTime = performance.now()
        ...
        const handleDark = async () => {
            ...
            const run = await createResponse({runID: localStorage.getItem("run_id"), screenID:1, selectedChoice:"Dark Pattern", rtms:performance.now() - startTtime });
            if (!run) return;
        ...
        const handleNudge = async () => {
            const run = await createResponse("started","Nudge");
            if (!run) return;
