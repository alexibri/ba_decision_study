...
export default function S1Dark1() {
    ...
    const createResponse = async (selected_choice) => {
    
            const newResponseData = {
                run_id: localStorage.getItem("run_id"),
                response_type:"",/*Was soll hier rein */
                response_rtms:Math.round(performance.now() - startTime),
                response_submitted_at:Date.now(),
                response_selected_choice:selected_choice
            }
        ...
        const selectDark = async () => {
            const run = await createResponse("Dark Pattern");
            if (!run) return;

            navigate("/scenario/1/step/3")
        }

        const selectNudge = async () => {
            const run = await createResponse("Nudge");
            if (!run) return;
    ...
    return <div className="s1screen fade-in">
        ...
        <div className="s1card">
            ...
            <div className="s1button">
                <button className="s1accept" onClick={selectDark} disabled={!enabled}>
                    Accept all
                </button>
            </div>
            ...
            <div>
                <button className="s1decline" onClick={selectNudge}>
                    Decline all
                </button>
            </div>
