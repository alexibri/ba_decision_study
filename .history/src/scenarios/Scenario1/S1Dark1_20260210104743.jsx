...
export default function S1Dark1() {
    ...
    const startTimeRef = useRefperformance.now()

        const handleDark = async () => {

            const run = await createResponse({runID: localStorage.getItem("run_id"), screenID:1, selectedChoice:"Dark Pattern", rtms:Math.round(performance.now() - startTime) });
            if (!run) return;
