...
export default function S1Dark2() {
    ...
    const startTime = performance.now()
            ...
            const handleDark = async () => {
                ...
                const run = await createResponse({runID: localStorage.getItem("run_id"), screenID:1, selectedChoice:"Dark Pattern", rtms:performance.now() - startTime });
                if (!run) return;
            ...
            const handleNudge = async () => {
                const run = await createResponse({runID: localStorage.getItem("run_id"), screenID:1, selectedChoice:"Nudge", rtms:performance.now() - startTime });
                if (!run) return;
