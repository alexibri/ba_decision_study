...
export default function S1Dark1() {
    ...
    const startTimeRef = useRef(performance.now())
        ...
        const handleDark = async () => {
            const currentRtms = Math.round(performance.now() - startTime) 
            const response = await createResponse({runID: localStorage.getItem("run_id"), screenID:1, selectedChoice:"Dark Pattern", rtms:currentRtms});
            if (!response) return;
        ...
        const handleNudge = async () => {
            const currentRtms = Math.round(performance.now() - startTimeRef.cu) 
            const response = await createResponse({runID: localStorage.getItem("run_id"), screenID:1, selectedChoice:"Nudge", rtms:currentRtms});
            if (!response) return;
    ...
    return <div className="s1screen fade-in">
        ...
        <div className="s1card">
            ...
            <div className="s1button">
                <button className="s1accept" onClick={handleDark}>
                    Accept all
                </button>
            </div>
            ...
            <div>
                <button className="s1decline" onClick={handleNudge}>
                    Decline all
                </button>
            </div>
