...
export default function S1Dark1() {
    ...
    const handleNudge = async () => {
        const currentRtms = Math.round(performance.now() - startTimeRef.current)
        const response = await createResponse({ runID: Number(localStorage.getItem("run_id")), screenID: 1, selectedChoice: "Nudge", rtms: currentRtms });
        if (!response) return;
    ...
    return <div className="s1screen fade-in">
        ...
        <div className="s1card">
            ...
            <div className="s1button">
                <button className="s1accept" onClick={() => {handleChoice("Dark Pattern","/scenario/1/step/3")}}>
                    Accept all
                </button>
            </div>
            ...
            <div>
                <button className="s1decline" onClick={handleChoice("Nudge","/scenario/1/step/3")}>
                    Decline all
                </button>
            </div>
