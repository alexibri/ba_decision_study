...
export async function getActiveRunForUser ({userID, currentesignStrategy}) {
    ...
    if(designStrategy != null){
        userQuery = userQuery.eq("design_strategy_id", designStrategy)
    }

    const {data, error} = await userQuery.maybeSingle()

    if(error){
        console.log("Error fetching active run:", error)
        return null
    }
