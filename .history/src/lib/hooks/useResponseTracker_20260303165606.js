...
export async function sendResponsePayload(currentRunID, currentScreenID, currentSelectedChoice, currentReationTime) {
    const response = await createResponse(
        {
            runID: currentRunID,
            screenID: currentScreenID,
            selectedChoice: currentSelectedChoice,
            rtms: currentReationTime
        });
...
export function useResponseTracker(screenID) {
    ...
    const handleChoice = async (choice, navigateTo) => {
        await runOnce(async () => {
            ...
            const currentReationTime = calcRtMs(startTimeRef.current, performance.now())

            let approved = false
            try {
                approved = await sendResponsePayload(runID, screenID, choice, currentReationTime)
            } catch {
                return
            }
