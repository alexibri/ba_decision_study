...
export async function sendResponsePayload(currentRunID, currentScreenID, currentSelectedChoice, current) {
...
export function useResponseTracker(screenID) {
    ...
    const handleChoice = async (choice, navigateTo) => {
        await runOnce(async () => {
            ...
            const current = calcRtMs(startTimeRef.current, performance.now())

            let approved = false
            try {
                approved = await sendResponsePayload(runID, screenID, choice, current)
            } catch {
                return
            }
