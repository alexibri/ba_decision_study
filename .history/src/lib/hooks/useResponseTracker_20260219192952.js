...
function calcRtMs (start,now) {
    return Math.round(now - start)
}

async function buildResponsePayload (currentRunID, currentScreenID, currentSelectedChoice, currentRtMs) {
    ...
    return Boolean(!response.approved) return;
}
