...
CREATE TABLE IF NOT EXISTS response(
    ...
    CONSTRAINT uk_response_session_screen_id UNIQUE (run_id, screen_id)

);

CREATE INDEX idx_response_session ON response(run_id);
CREATE INDEX idx_response_screen  ON response(screen_id);
CREATE INDEX idx_session_participant ON run(participant_id);