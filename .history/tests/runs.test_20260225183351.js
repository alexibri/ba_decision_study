...
import { createRun, updateRun, getRunById, getActiveRunForUser } from "../src/lib/db/runs";
...
describe("runs.createRun", () => {
    ...
    it("returns null on insert error", async () => {
        authGetUserMock.mockResolvedValueOnce({ data: { user: { id: "user_test" } }, error: null, })
        builder.single.mockResolvedValueOnce({ data: null, error: "fail" })
...
describe("runs.updateRun", () => {
    ...
    it("updates run_status + run_end_at and returns run_id", async () => {
        ...
        expect(builder.update.mock.calls[0][0][0]).toMatchObject({
            run_end_at: expect.any(String),
            run_status: "finished",
        })
        expect(builder.eq).toHaveBeenCalledWith("run_id", 1)
        expect(builder.select).toHaveBeenCalledWith("run_id")
        expect(builder.single).toHaveBeenCalled()
...
describe("runs.getRunById", () => {

    it("queries by run_id and return a row", async () => {
        builder.maybeSingle.mockResolvedValueOnce({ data: { run_id: 1, run_status: "started" }, error: null })

        const response = await getRunById(1)
        ...
        expect(response).toEqual({ run_id: 1, run_status: "started" })
    })
})

describe("runs.getActiveRunForUser", () => {

    it("queries filters and returns row", async () => {
        builder.maybeSingle.mockResolvedValueOnce({ data: { run_id: 1, user_id: 2, design_strategy_id: 1, run_status: "started" }, error: null })

        const response = await getActiveRunForUser({ userID: 2, currentDesignStrategy: 1 })

        expect(fromMock).toHaveBeenCalledWith("run")
        expect(builder.select).toHaveBeenCalled()
        expect(builder.eq).toHaveBeenCalledWith("user_id", 2)
        expect(builder.eq).toHaveBeenCalledWith("run_status", "started")
        expect(builder.is).toHaveBeenCalledWith("run_end_at", null)
        expect(builder.order).toHaveBeenCalledWith("run_id", { ascending: false })
        expect(builder.limit).toHaveBeenCalledWith(1)

        expect(builder.eq).toHaveBeenCalledWith("design_strategy_id", 1)
        expect(builder.maybeSingle).toHaveBeenCalled()

        expect(response).toEqual({ run_id: 1, user_id: 2, design_strategy_id: 1, run_status: "started" })
    })
})
