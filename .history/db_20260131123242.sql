...
CREATE TABLE IF NOT EXISTS response(
    ...
    CONSTRAINT uk_response_screen_id UNIQUE (run_id, screen_id)
);

CREATE INDEX idx_response ON response(run_id);
CREATE INDEX idx_response_screen  ON response(screen_id);
CREATE INDEX idx_participant ON run(participant_id);