...
GRANT EXECUTE ON FUNCTION finish_run(INT) 
...
ON run FOR INSERT TO authenticated
...
ON response FOR INSERT TO authenticated
WITH CHECK (
    EXISTS (
        SELECT 1 FROM run
        WHERE run.run_id = response.run_id
        AND run.user_id = auth.uid()
    )
);

CREATE POLICY "Public can read design strategies"
ON design_strategy FOR SELECT TO anon, authenticated
USING (true);

CREATE POLICY "Public can read screens"
ON screen FOR SELECT TO anon, authenticated
USING (true);
