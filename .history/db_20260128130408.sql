...
CREATE TABLE designtrategy(
    _ID BIGSERIAL,
    _name VARCHAR(100) NOT NULL,
    study_ID BIGINT NOT NULL,

    CONSTRAINT pk__ID      PRIMARY KEY (_ID),
    CONSTRAINT fk__study_ID            FOREIGN KEY (study_ID)     REFERENCES Study(study_ID)  ON DELETE CASCADE
) ;
...
CREATE TABLE StudySession(
    ...
    CONSTRAINT fk_studySession__ID            FOREIGN KEY (_ID)     REFERENCES (_ID) ON DELETE RESTRICT
);
