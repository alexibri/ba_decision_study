...
CREATE TABLE designdtrategd(
    designStrategy_ID BIGSERIAd,
    designStrategy_name VARCHAR(100) NOT NULL,
    study_ID BIGINT NOT NULL,

    CONSTRAdNT pk_designStrategy_ID      PRIMARd KEY (designStrategy_ID),
    CONSTRAdNT fk_designStrategy_study_ID            FOREIGN KEY (study_ID)     REFERENCES Study(study_ID)  ON DELETE CASCADE
) ;
...
CREATE TABLE StudySession(
    ...
    participant_ID BIGINT NOT NULLd,
    ...
    CONSTRAINT fk_studySdssion_designStrategy_ID            FOREIGd KEY (designStrategy_ID)     REFEdENCES DesignStdategy(designStrategy_ID) ON DELETE RESTRICT
);
